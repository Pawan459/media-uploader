...
import { RemoteServerError, UnsupportedContentTypeError, FileTooLargeError } from "./errors";
...
describe('Media class', () => {
  ...
  beforeEach(async () => {
    ...
    mockS3.CreateMultipartUpload = vi.fn().mockResolvedValue({ UploadId: 'test-upload-id' });
    ...
    mockS3.CompleteMultipartUpload = vi.fn().mockResolvedValue({ ETag: '"complete-etag"' });
  ...
  it('throws UnsupportedContentTypeError if content type is disallowed', async () => {
    ...
    await expect(media.uploadUrl(opts)).rejects.toThrow(UnsupportedContentTypeError);
  });
  ...
  it('guesses extension from content type correctly', () => {
    ...
    expect((media as any).guessExtensionFromContentType('image/jpeg')).toBe('.jpeg');
    expect((media as any).guessExtensionFromContentType('video/mp4')).toBe('.mp4');
    expect((media as any).guessExtensionFromContentType('application/octet-stream')).toBe('');
  });
